...
Graph.prototype.addVertex = function(id, passthrough) {
...
Graph.prototype.addSourceVertex = function(id, passthrough, cls) {
	let idx = this.registerAlias(id);
	if(idx != null && this.vertices[idx] == null) {
...
Graph.prototype.addEdge = function(frm, to){
...
Graph.prototype.registerAlias = function(id) {
...
Graph.prototype.aliasLookup = function(id) {
...
Graph.prototype.setPassthrough = function(id, val) {
...
$.getJSON("mapInfo.json", function(json) {
	let graphData = json.graphData;
	diagramGraph = generateGraph(graphData);
	$('.closeable').click(e => {
		$('#' + e.target.id).toggleClass('closed');
		diagramGraph.setPassthrough(e.target.id); //toogle
		diagramGraph.update();
		diagramGraph.draw();
	});
	diagramGraph.update();
